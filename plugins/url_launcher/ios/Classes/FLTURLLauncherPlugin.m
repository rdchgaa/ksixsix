...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    if ([@"canLaunch" isEqualToString:call.method]) {
        ...
        NSRange range = [[url lowercaseString] rangeOfString:@"http" options:NSBackwardsSearch];
        if(0 != range.location){
            [self openFile: url result:result];
            return;
        }
...
- (CGRect)documentInteractionControllerRectForPreview:(UIDocumentInteractionController*)controller {
        return [self topViewController].view.frame;
}

-(void)openFile:(NSString*)fileUrl result:(FlutterResult)result{
    UIDocumentInteractionController* _document =  [UIDocumentInteractionController interactionControllerWithURL:[NSURL fileURLWithPath:fileUrl]];
    _document.delegate = self;
   
    [_document presentPreviewAnimated:YES]; // 预览文件
  // [_document presentOptionsMenuFromRect:self.view.bounds inView:self.view animated:YES]; // 菜单操作
